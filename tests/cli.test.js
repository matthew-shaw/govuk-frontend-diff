...
describe('govuk-frontend-diff cli', () => {
  ...
  it('checks all pass when passing the Nunjucks reference templates back through it', async (done) => {
    const [server, port] = await runExampleServer('v5.0.0');

    const { exitCode, stdout, stderr } = await run(
      `http://localhost:${port} --govuk-frontend-version=v5.0.0`
    );
    expect(stdout + stderr).toMatchSnapshot();
  ...
  it('all checks fail when passing nonsense output through it', async (done) => {
    ...
    const { exitCode, stdout, stderr } = await run(
      `http://localhost:${port} --hide-diffs --govuk-frontend-version=v5.0.0`
    );
    expect(stdout + stderr).toMatchSnapshot();
    expect(exitCode).toEqual(1);
  ...
  it('can run a subset of the checks by excluding some', async (done) => {
    ...
    const { exitCode, stdout, stderr } = await run(
      `http://localhost:${port} --govuk-frontend-version=v5.0.0 --exclude button accordion page-template`
    );
    expect(stdout + stderr).toMatchSnapshot();
    expect(exitCode).toEqual(0);
  ...
  it('can run a subset of the checks by explicitly including some', async (done) => {
    ...
    const { exitCode, stdout, stderr } = await run(
      `http://localhost:${port} --govuk-frontend-version=v5.0.0 --include button page-template`
    );
    expect(stdout + stderr).toMatchSnapshot();
    expect(exitCode).toEqual(0);
  ...
  it('throws an error if the passed render script throws an error', async (done) => {
    const [server] = await runExampleServer('v5.0.0');

    const { exitCode, stdout, stderr } = await run(
      `http:// --govuk-frontend-version=v5.0.0`
    );
  ...
  it('displays html diffs', async (done) => {
    ...
    const { exitCode, stdout, stderr } = await run(
      `http://localhost:${port} --govuk-frontend-version=v5.0.0 --include button`
    );
    expect(stdout + stderr).toMatchSnapshot();
    expect(exitCode).toEqual(1);
  ...
  it('hides html diffs when requested', async (done) => {
    ...
    const { exitCode, stdout, stderr } = await run(
      `http://localhost:${port} --govuk-frontend-version=v5.0.0 --include button --hide-diffs`
    );
    expect(stdout + stderr).toMatchSnapshot();
    expect(exitCode).toEqual(1);
